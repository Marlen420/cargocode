...
export class MapboxService {
  ...
  private readonly calculateDistanceUrl: string =
    CONFIG.mapboxDistanceCalculationurl;
  ...
  async getGeocodeAddress(
    ...
    const response = await axios.get(
      `${this.locationFromAddressUrl}${encodeURIComponent(
        address,
  ...
  async getDistance(
    ...
    try {
      const startResponse = await this.getGeocodeAddress(
        startAddress,
        'string',
      );
      ...
      const response = await axios.get(
        `${this.calculateDistanceUrl}${startResponse};${endResponse}?access_token=${this.accessToken}`,
      );

      distanceInMeter = response.data.routes[0].distance;
      await this.redisService.set(DISTANCE_IN_METER_KEY, distanceInMeter);
      const distanceInUnit = Math.ceil(
        distanceInMeter / METER_CONVERT_VALUES[unit],
      );

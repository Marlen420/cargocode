...
export class CompaniesService {
    ...
    async findById(id: number): Promise<CompanyEntity> {
        const company: CompanyEntity =  await this.redisService.get(`companies:companiesService:company:${id}`);
        if (company) {
            return company;
        }
    ...
    async createCompany(data: CreateCompanyDto): Promise<CompanyEntity> {
        let company: CompanyEntity = await this.companyRepository.findOne({where: {login: data.login}});
        if (company) {
            throw new BadRequestException('This login already taken');
        }
        company = await this.companyRepository.findOne({where: {email: data.email}});
        if (company) {
     ...
     */
    async addEmployee(req: Request, data: AddEmployeeDto): Promise<CompanyEntity> {
        const token: UserToken = this.getDecodedToken(req);
        if (!data.email && !data.phone) {
            throw new BadRequestException('Must be provided at least one option');
        }
        let company: CompanyEntity = await this.redisService.get(`companies:companiesService:company:${token.id}`);
        if (!company) {
            company = await this.companyRepository.findOne({where: {id: token.id}})
        }
        if (company.employees_credential.some((item: AddEmployeeDto) => item.email === data.email || item.phone === data.phone)) {
        ...
        return this.companyRepository.save(company)
            .then(async (savedCompany) => {
                await this.redisService.set(`companies:companiesService:company:${token.id}`, savedCompany);
                return savedCompany;
            });
    }

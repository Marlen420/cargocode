...
export class OrdersService {
    ...
    async priceEstimate(data: PriceEstimateDto) {
        const distance = await this.mapboxService.getDistance(data.pickup_location, data.destination, DistanceUnit.mi);
    ...
    async createOrder(req: Request, data: CreateOrderDto): Promise<any> {
        ...
        const shipper: ShipperEntity = await this.usersService.findOneShipper(token.id);
        if (!shipper) {
        ...
        return this.orderRepo.save(order)
          .then(async (savedOrder) => {
						await this.redisService.set(`orders:ordersService:order:${savedOrder.id}`, savedOrder);
						return savedOrder;
					});
    }
		...
		async acceptOrder(req: Request, orderId: number): Promise<any> {
			const token = this.getDecodedToken(req);
			let order: OrderEntity = await this.redisService.get(`orders:ordersService:order:${orderId}`);
			if (!order) {
			...
			return this.orderRepo.save(order)
				.then(async (savedOrder) => {
					await this.redisService.set(`orders:ordersService:order:${orderId}`, savedOrder);
					return savedOrder;
				});
		}
		...
		async startShipping(req: Request, id: number): Promise<OrderEntity> {
			const token = this.getDecodedToken(req);
			let order: OrderEntity = await this.redisService.get(`orders:ordersService:order:${id}`);
			...
			return this.orderRepo.save(order)
				.then(async (savedOrder) => {
					await this.redisService.set(`orders:ordersService:order:${id}`, savedOrder);
					return savedOrder;
				})
		}
		...
		async disableOrder(req: Request, id: number) {
			...
			if (!order) {
				order = await this.orderRepo.findOne({where: {shipper: token.shipper, id}})
				if (!order) {
					throw new BadRequestException('Order doesn\'t exist');
				}
			}
    ...
    async getOrderById(id: number): Promise<OrderEntity> {
        const order: OrderEntity = await this.redisService.get(`orders:ordersService:order:${id}`);
        if (order) {
            return order;
        }
		...
		async getMyOrders(req: Request): Promise<OrderEntity[]> {
			...
			if (token.role === RolesEnum.CARRIER) {
				const carrier = await this.usersService.findOneCarrier(token.id, {user: false});
				if (!carrier) {
		...
		private getDecodedToken(req: Request): UserToken {
			return this.jwtService.decode(req.headers.authorization.split(' ')[1]) as UserToken;
		}
}